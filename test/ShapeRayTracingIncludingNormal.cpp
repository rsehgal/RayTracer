	...
	for (int j = ny - 1; j >= 0; j--) {
		for (int i = 0; i < nx; i++) {
			...
			if(dist < kInfLength){
				...
				countHits++;
				//Vector3D<double> N = (r.PointAtDistance(dist) - Vector3D<double>(0.,0.,-1)).Unit();
				Vector3D<double> N(0.,0.,0.);
				orbPlaced->Normal(r.PointAtDistance(dist),N);
				col = 0.5*Vector3D<double>(N+1);//.Set(1.,0.,0.);
			}
			else{
				col = r.Color();
			}
